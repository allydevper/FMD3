...
fn scan_and_cache_roundtrip() {
    ...
    let t0 = Instant::now();
    let first = fmd3_lib::lua_host::modules_list();
    ...
    let t1 = Instant::now();
    let n = fmd3_lib::lua_host::modules_refresh();
    println!(
    ...
    let after = fmd3_lib::lua_host::modules_list();
    ...
    for meta in first.iter().take(50) {
        let by_id = fmd3_lib::lua_host::find_by_id(&meta.id)
        ...
        let hits = fmd3_lib::lua_host::modules_match_url(&meta.root_url);
        assert!(
